...
export class OpAperiodiqueComponent implements OnInit {
            ...
            if (obsMP.status == 200) {
                ...
                that.ij2srvc.listTecOpTempWS(data, this.idToken).subscribe(obs => {
                    if (obs.status == 200 && obs.body != null) {
                        ...
                        that.OP.fileAttente = _fileAttente;
    ...
    creeFileAttenteOP() {
        ...
        this.OP.totalOP = 0;
        for (const file of this.OP.fileAttente.tecop.tecbenef) {
            if (file.id_mode_paiement) {
                if (file.id_mode_paiement.abrevModePaiement == this.OP.abrevMP) {
    ...
    creerArgumentOp() {
        ...
        const that = this;
        // this.directionService.infoDirectionWS(this.utilisateur.id_acces, this.idToken).subscribe(obs => {
        //     if (obs.status == 200 && obs.body != null) {
        //         argOp.dr = obs.body.code_dr.dr_code;
        //         argOp.tecop.code_dr_service = obs.body.code_dr.dr_code;
    ...
    genererOP(argOp) {
        ...
        const that = this;
        // this.rfaService.addTecopWS(argOp, this.idToken).subscribe(obsOp => {
        //     if (obsOp.status == 200) {
...
//     const that = this;
//     this.directionService.infoDirectionWS(this.utilisateur.id_acces, this.idToken).subscribe(
//       obsDrWS => {
//         if (obsDrWS.body.code_fonction.code_fonction == "02A") {
//           that.estDirecteur = true;
//         }
//         that.dr = obsDrWS.body.code_dr.dr_code;
//         if (that.code_dr == null || that.code_dr == undefined) {
//           that.code_dr = that.dr;
//         }
//         that.suiteNgOnInit(obsDrWS.body.code_dr.dr_code);
//       },
//       err => {
...
//       this.afficheChoixProjet();
//     }
//     this.banqueService.listeMPvalideWS(this.idToken).subscribe(obsMP => {
//       if (obsMP.status == 200) {
//         that.OP.mpChoisi = obsMP.body[0];
//         that.OP.abrevMP = obsMP.body[0].abreviation;
//         that.OP.listeMP = obsMP.body;
...
//           dr: dr
//         };
//         that.rfaService.listTecOpTempWS(data, this.idToken).subscribe(obs => {
//           if (obs.status == 200 && obs.body != null) {
//             const _fileAttente = obs.body;
//             for (const opTemp of _fileAttente['tecop'].tecbenef) {
//               opTemp.nomBenef = '';
//               opTemp.prenomBenef = '';
//               opTemp.cinBenef = '';
//               that.infoService.infoIndivWebService(opTemp.id_benef).subscribe(
//                 obsIWS => {
//                   if (obsIWS.status == 200 && obsIWS.body != null) {
//                     opTemp.nomBenef = obsIWS.body['nom'];
//                     opTemp.prenomBenef = obsIWS.body['prenoms'];
...
//                   }
//                   else {
//                     this.employeurService.infoEmployeurWS(opTemp.id_benef, this.idToken).subscribe(empl => {
//                       opTemp.nomBenef = empl.body.employeur_nom;
//                     });
//                   }
//                 });
...
//     // $(this.modalChoixProjet.nativeElement).modal('show');
//     const that = this;
//     this.budgetService.budgetTopicWS('listeProjetPourElaborationBPSE', '', this.idToken).subscribe(obsP => {
//       if (obsP.status == 200 && obsP.body != null && obsP.body['length'] > 0) {
//         that.OP.listeProjet = <any>obsP.body;
//       } else {
//         that.toast.info('Impossible de récupérer la liste des projets');
...
//         argOp.dr = obs.body.code_dr.dr_code;
//         argOp.tecop.code_dr_service = obs.body.code_dr.dr_code;
//         that.genererOPD(argOp, indice);
//       } else {
//         that.toast.info('Votre service est non reconnu, veuillez réessayer');
//         that.OPD.charge = false;
//       }
//     });
//   }

//   genererOPD(argOp, indice: number) {
//     const argDmd = {
//       listeIdAcc: [],
//       numOP: null,
//     };
//     argDmd.listeIdAcc.push(this.OPD.fileAttenteOP[indice].tecop.tecbenef[0].id_demande);
...
//         this.resultSave.emit(obsOp.body);
//         that.toast.success('OP généré avec succès');
//         for (let i = 0; i < that.OPD.fileAttente.tecBenef.length; i++) {
//           if (that.OPD.fileAttente.tecBenef[i].tecop.tecbenef[0].id_demande == this.OPD.fileAttenteOP[indice].tecop.tecbenef[0].id_demande) {
//             that.OPD.fileAttente.tecBenef.splice(i, 1);
//             break;
//           }
//         }
//         that.OPD.fileAttenteOP.splice(indice, 1);
//         argDmd.numOP = obsOp.body['tecop'].id_op;
//         that.rfaService.insertionOpDmdGroupe(argDmd, that.prestation).subscribe(obs => {
//           console.log("insertionOpDmdGroupe", obs);
//         });
...
//         this.budgetService.getCptbyGroupeWS(this.idRubrique, this.idToken).subscribe(obsWS => {
//           let obs = this.ui.transformeWSReponse(obsWS);
...
//         };
//         let that = this;
//         this.rfaService.listTecOpTempSupWS(data, this.idToken).subscribe(obs => {
//           if (obs.status == 200 && obs.body != null) {
...
//               opTemp.prenomBenef = '';
//               opTemp.cinBenef = '';
//               that.infoService.infoIndivWebService(opTemp.tecop.tecbenef[0].id_benef).subscribe(obsIWS => {
//                 if (obsIWS.body != null) {
//                   opTemp.nomBenef = obsIWS.body['nom'];
//                   opTemp.prenomBenef = obsIWS.body['prenoms'];
//                   opTemp.cinBenef = obsIWS.body['cin'];
...
//     const that = this;
//     this.Liste.liste = [];
//     this.ijPfService.listTecopbyprestationWS(this.Liste.filtre, this.idToken).subscribe(obs => {
...
//       const that = this;
//       if (numOp.length >= 10) {
//         this.ijPfService.listTecopByIdWS(numOp, this.idToken).subscribe(obs => {
//           if (obs.status == 200 && obs.body != null) {
...
//               that.infoService.infoIndivWebService(obs.body.tecbenef[0].id_benef).subscribe(obsI => {
//                 if (obsI.status == 200) {
//                   that.texteBenef = obsI.body['nom'] + " " + obsI.body['prenoms'];
//                 }
//                 else {
//                   this.employeurService.infoEmployeurWS(obs.body.tecbenef[0].id_benef, this.idToken).subscribe(empl => {
...
//   affichePieces(ref: string) {
//     this.DetailPiece.reference = ref;
//     this.DetailPiece.liste = [];
//     let that = this;
//     let fileQuery = new FileModel();
//     fileQuery.id_files = ref;
//     if (ref[0] == "4") {
//       this.fileService.readQueryWS(fileQuery).subscribe(data => {
//         if (data.status == 200 && data.body != null && data.body['length'] > 0) {
//           that.DetailPiece.liste = <any>data.body;
//         }
//       });
//     }
//     else {
...
//   ouvrirFichier(piece) {
//     this.ui.telechargeDataURL(document, piece.name, piece.file);
//   }
